...
function countReadOrUnReadNum(){
	...
	countParam.mainObjectType="ORDER";
	 postUtil.call("/mallOrder/api/orderStatusCount", countParam, function (result) {
		    ...
		    if(deliveryWaitingNum>0){
		    	$(".tab-nav li[tabname=DELIVERY_WAITING]").addClass(numCss).html('待发货<i>'+deliveryWaitingNum+'</i>');
		    }else{
		    	$(".tab-nav li[tabname=DELIVERY_WAITING]").removeClass(numCss).html('待发货');
		    }
		    if(receivingWaitingNum>0){
		    	$(".tab-nav li[tabname=RECEIVE_WAITING]").addClass(numCss).html('待收货<i>'+receivingWaitingNum+'</i>');
		    }else{
		    	$(".tab-nav li[tabname=RECEIVE_WAITING]").removeClass(numCss).html('待收货');
		    }
		    if(completedNum>0){
		    	$(".tab-nav li[tabname=COMPLETED]").addClass(numCss).html('交易完成<i>'+completedNum+'</i>');
		    }else{
		    	$(".tab-nav li[tabname=COMPLETED]").removeClass(numCss).html('交易完成');
		    }
		    if(closedNum>0){
		    	$(".tab-nav li[tabname=CLOSED]").addClass(numCss).html('已关闭<i>'+closedNum+'</i>');
		    }else{
		    	$(".tab-nav li[tabname=CLOSED]").removeClass(numCss).html('已关闭');
		    }
		    if(totalNum>0){
		    	$(".ALLNUM").addClass(numCss).html('全部<i>'+totalNum+'</i>');
		    	if(!$('span', parent.document).hasClass('mallOrderListOfPlatForm')){
		    		$('a[href-url="/mallOrder/api/preList"]', parent.document).append('<span name="mallOrderListOfPlatForm" class="mallOrderListOfPlatForm has-num" style="margin-left:14px;"><i class="mallOrderListOfPlatFormCount">'+totalNum+'</i></span>');
		    	}else{
		    		$('.mallOrderListOfPlatFormCount', parent.document).html(totalNum);
		    	}
		    }else{
		    	$(".ALLNUM").removeClass(numCss).html('全部');
		    	$('span[name="mallOrderListOfPlatForm"]', parent.document).remove();
		    }
	 })
}
function loadMallOrderList(param) {
	layer.load();
    postUtil.call("/mallOrder/api/list", param, function (result) {
        ...
        if (length > 0) {
            for (var i1 = 0; i1 < dataList.length; i1++) {
                ...
                html+='	<div class="enquriry-data mb20" state="待发货">      '                                                                                    
				            +'<ol>'
				            +'  <li class="fw-b"><input class="'+readClass+'" type="checkbox" name="single" orderId="'+isExitsVariable(data.orderId)+'"><span>买方账号：<span class="c-f">'+isExitsVariable(data.buyerUserMobile)+'/'+isExitsVariable(data.buyerUserRealName)+'</span></span></li>'
				            +'   <li class="fw-b"><span>创建时间：<span class="fw-b">'+moment(data.createTime).format("YYYY-MM-DD HH:mm:ss")+'</span></span></li>'
				            +'    <li style="width:27%"><span class="c-9">联系人/联系电话：'+isExitsVariable(data.entrustOrderContact)+'/'+isExitsVariable(data.entrustOrderContactMobile)+'</span></li>'
				            +'    <li style="float:right;width:20%;text-align:right"><span class="c-9">专属客户经理：'+isExitsVariable(data.accountManagerName)+'</span></li>'
				            +'     <li class="ml20" style="width:30%"><span>卖方账号：<span class="c-f">' + data.sellerAccountName + '</span></span></li>'
				            +'     <li style="float:right;width:17%;text-align:right"><span class="c-9">订单编号：'+isExitsVariable(data.orderId)+'</span></li>'
				            +'</ol>     '
                            	 ...
                            	 for(var i=0;i<mallOrderDetailList.length;i++){
                            		 ...
                            		 html+=' <tr>  ';
			                 	          if(skuResource=="1"){
			                 	        	...
			                 	        	+       '    <td><a href=\"javascript:void(0);\" onclick="goProductPreview(\''+mallOrderDetailList[i].skuId+'\');"><p  title="'+isExitsVariable(mallOrderDetailList[i].skuName)+'">'+isExitsVariable(mallOrderDetailList[i].skuName)+'</p></a></td>';
			                 	        	html+=   '    <td>'+isExitsVariable(mallOrderDetailList[i].skuMode)+'</td>'+
			                 	            '    <td>'+isExitsVariable(mallOrderDetailList[i].skuBrandName)+'</td>'+
			                 	            '    <td class="unitDisplay" unitName="'+isExitsVariable(mallOrderDetailList[i].skuUnitName)+'" >'+isExitsVariable(mallOrderDetailList[i].skuQuantity)+'</td>'+
			                 	            '    <td>'+isExitsVariable(mallOrderDetailList[i].skuUnitName)+'</td>';
			                 	          }else{
			                 	        	 ...
			                 	        	 html+= '    <td style="text-align:left!important;">自定义商品</td>'
			                 	        	     +  '    <td><p  title="'+isExitsVariable(mallOrderDetailList[i].skuName)+'">'+isExitsVariable(mallOrderDetailList[i].skuName)+'</p></td>';
			                 	        	html+=   '    <td>'+isExitsVariable(mallOrderDetailList[i].skuMode)+'</td>'+
			                 	            '    <td>'+isExitsVariable(mallOrderDetailList[i].skuBrandName)+'</td>'+
			                 	            '    <td>'+skuQuantity+'</td>'+
			                 	            '    <td>'+isExitsVariable(mallOrderDetailList[i].skuUnitName)+'</td>';
			                 	            ...
			                 	            if(i==0){
					                             ...
					                             '    <ul>'+
					                             '     <li>'+isExitsVariable(data.corporationAddress)+'</li>'+
					                             '      <li>'+isExitsVariable(data.consignee);
					                           var consigneeMobile=isExitsVariable(data.consigneeMobile);
					                             if(consigneeMobile!=''){
					                            	 html +='/'+isExitsVariable(data.consigneeMobile)+'</li>';
					                             }else{
					                            	 html +='</li>';
					                             }
					                      html+='    </ul>'+
					                            ...
					                            '  <dd class="flt">'+
					                            '     <ul>'+
					                            '      <li>'+isExitsVariable(data.inquiryOrderProvince)+'</li>'+
					                            '     <li>'+isExitsVariable(data.inquiryOrderCity)+'</li>'+
					                            '   </ul>'+
					                            ' </dd>'+
					                            ' </dl> '+
					                            ' </td>'+
				                 	            '    <td rowspan='+mallOrderDetailList.length+'>'+
				                                '    <dl>'+
				                                '       <dt class="c-f fz14 fw-b mb5">'+(data.totalOrderAmount ? data.totalOrderAmount.toFixed(2) : "0.00")+'元<dt>'+
				        					  ...
				        					  html+= '</dd>'+
				                               ...
				                               '   </td>'+
				                 	           '    <td rowspan='+mallOrderDetailList.length+'>'+
				                               '     <dl>'+
				                                '     <dt class="status mt10">'+isExitsVariable(data.statusCname)+'</dt>';
				                              
				                                if("DELIVERY_WAITING"==status){
				                                	 html+=	'      <dd style="color:#999;">'+isExitsVariable(data.updateTime)+'</dd>';
				                                }else{
				                                	 html+=	'      <dd style="color:#999;">'+isExitsVariable(data.updateTime)+'</dd>';
				                                }
				                                ...
				                                if(deliveryLogList!=null&&deliveryLogList!=""){
			                 	            	if("RECEIVE_WAITING"==status||"COMPLETED"==status){
						                             html+=  '        <dd class="express-detail">'+
								                       ...
								                       '          <li>发货:'+isExitsVariable(deliveryLogList.deliveryTime)+'</li>'+
								                       '           <li>物流公司:'+isExitsVariable(deliveryLogList.transportCorporationName)+'</li>'+
								                       '           <li>单号:'+isExitsVariable(deliveryLogList.transportNo)+'</li>'+
								                       '          <li>物流公司联系人:'+isExitsVariable(deliveryLogList.contactsName)+'</li>'+
								                       '           <li>联系方式:'+isExitsVariable(deliveryLogList.contactsMobileOrFixedPhoneNumber)+'</li>'+
								                       '         </ul>'+
								                       '        </dd>';
					                               }
			        						 ...
			        						 if ("RECEIVE_WAITING" == status) {
			                 	            		html+=( '<p><div class="inline colockbox" startTime="'+new Date(data.deliveryTime)+'" currentTime="'+new Date(data.currentTime)+'">	'+	
			                 	            ...
			                 	            html+=  '    </dl>          '+        
				                 	            '    </td>'+
				                                '  <td rowspan='+mallOrderDetailList.length+'>'+
				                                '    <span class="special-button mb10 mallOrderBaseDetail" orderId="'+isExitsVariable(data.orderId)+'">查看</span>';
				                                if("10002"==mainObjectId){
				                                    if("DELIVERY_WAITING"==status){
				                                	//平台允许发货
						                              html+= '   <dl>'+
							                                 '    <dd><button class="form-btn layui-btn-itoc layui-btn-small3 mb5 deliveryMallOrderBase"" lay-filter="formDemo" orderId="'+isExitsVariable(data.orderId)+'">发货</button></dd>'+
							                                 '   </dl>'+
						                                	 '<span class="special-button close mt10 closeMallOrderBase" orderId="'+isExitsVariable(data.orderId)+'">关闭</span>';
				                                	}
				                                }
												if("IN_CONFORMING_PLATFORM"==status){
													html+= '   <button class="form-btn layui-btn-itoc layui-btn-small3 mt10 mallOrderBaseDetail" style="width: 100px;" lay-filter="formDemo"  orderId="'+isExitsVariable(data.orderId)+'" indexHtml=confirmHTML'+i1+'>选择分销商</button></dd>          ';
												}
...
//查看详情
$(document).on("click",".mallOrderBaseDetail",function(){
	var orderId=$(this).attr("orderId");
	...
	readParam.type="READ";
	postUtil.call("/mallOrder/api/orderReadNoRead", readParam,function(){
		loadMallOrderList(param);
	});
})
...
//已读
$(document).on('click','.readSet',function () {
	var orderIdArr="";
	...
	readParam.type="READ";
	postUtil.call("/mallOrder/api/orderReadNoRead", readParam,function(result){
		if (result.STATUS == 'SUCCESS') {
			 countReadOrUnReadNum();
...
//未读
$(document).on('click','.noReadSet',function () {
	var orderIdArr="";
	 $("input[type=checkbox][name=single]:checked").each(function(index,item){
		 var orderId=$(item).attr("orderId");
		 orderIdArr+=(orderId+",");
