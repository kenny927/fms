    ...
    $.fn.extend({
        MSDL: function (options) {/*MultiSelectDropList*/
            ...
            return this.each(function () {
				...
				var defaultText = "";
				var defaultValues = "";
				options.defaultValue.forEach(function(item,index){
					if(options.defaultValue.length!=1){
                ...
                var $ipt = $('<input type="text" readonly id=\"' + options.id +'\"  value=\"'+ defaultText +'\" class="select_rel" />');//id="allTheValue"通过这个id取值
                var $ipt_hidden = $('<input type="hidden" value=\"'+ defaultValues +'\" id=\"' + options.id + "_" + "hidden" + '\" class="select_rel" />');
				...
				if(defaultText=="全部"){
				  var $all = $('<input type="checkbox" class="select_all" value="" checked/><label class="labelChoose">' + options.selectallTxt + '</label>');//全选 默认选中
				}else{
				  var $all = $('<input type="checkbox" class="select_all" value=""/><label class="labelChoose">' + options.selectallTxt + '</label>');//全选
				}
                ...
                for (var i = count - 1; i >= 0; i--) {
					...
					if(defaultText=="全部"){
					  $list = $('<div class="divChoose"><input type="checkbox" value="' + options.data[i].value + '" checked /><label class="labelChoose">' + options.data[i].text + '</label><br></div>');
					}else{
					  					  
					  for(var j=0;j<options.defaultValue.length;j++){
						   if(options.defaultValue[j].value==options.data[i].value){
							$list = $('<div class="divChoose"><input type="checkbox" value="' + options.data[i].value + '"  checked/><label class="labelChoose">' + options.data[i].text + '</label><br></div>');  
							
							break;
						  }else{
							$list = $('<div class="divChoose"><input type="checkbox" value="' + options.data[i].value + '" /><label class="labelChoose">' + options.data[i].text + '</label><br></div>');    
						  }
					  }
					}
...
//				$this.addClass('multi_select_focus');
 //               });
				...
				$ipt.click(function () {
					if($(this).hasClass("select_rel_visited")){
						$container.addClass('hidden');
                        $ipt.removeClass("select_rel_visited")
					}else{
						$container.removeClass('hidden');
